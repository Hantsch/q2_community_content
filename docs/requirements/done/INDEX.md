...
- 017 — Validation panel in the studio · S04 · Added `studio/src/validate/build-validation-snapshot.ts` (shared composition now backing both `npm run validate` and the browser), `studio/src/validate/panel-model.ts`, the `ValidationPanel`/`SeverityBadge` organisms, a confined `provenance` bridge route plus browser client, and wired `StudioPage` to show the panel beside the library with a finding→entry jump and a re-check control — proven end to end by an e2e spec that shells out to the real `validate --json` CLI and compares it against the rendered panel (AC7).
